...
async function enterApp(withSpotify = false) {
  ...
  if (withSpotify) {
    // On navigue d'abord, puis on charge les données en arrière-plan
    showToast('Chargement de tes musiques…');
    try {
      // Pas besoin d'init SDK (pas de Premium) — juste les appels API REST
      await loadSpotifyData();
      updateSpotifySettingsUI(true);
      showToast('Musiques chargées ✓');
    } catch (e) {
      console.error('[enterApp] loadSpotifyData :', e);
      showToast('Impossible de charger tes musiques');
    }
  }
}
...
async function loadSpotifyData() {
  // 1. Écoutes récentes (avec preview URLs)
  const recent = await SpotifyManager.getRecentlyPlayed(20);
  if (recent.length) {
    State.recentTracks = recent;
    console.log('[Spotify] Écoutes récentes chargées :', recent.length);
  }

  // 2. Recommandations basées sur les écoutes récentes
  if (recent.length) {
    const seedIds = recent.slice(0, 5).map(t => t.id).filter(Boolean);
    if (seedIds.length) {
      const recos = await SpotifyManager.getRecommendations(seedIds);
      State.recoTracks = recos;
      console.log('[Spotify] Recos chargées :', recos.length);
    }
  }

  // 3. Genres → playlists auto
  const genres = await SpotifyManager.getTopGenres();
  State.playlists = genres;
  console.log('[Spotify] Genres chargés :', genres.length);
}
