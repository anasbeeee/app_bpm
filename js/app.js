/**
 * BPM SYNC — App.js Final
 * Spotify → écoutes récentes + recommandations perso
 * Deezer → covers + previews 30s
 * Fallback Deezer chart si pas de Spotify
 */
...
function coverPlaceholder(name, fontSize = '11px') {
  ...
  return `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:rgba(255,255,255,.5);font-size:${fontSize};font-weight:700">${initials}</div>`;
}
...
const Player = (() => {
  ...
  function playAudio(url, info, onEnd) {
    ...
    a.oncanplaythrough = () => {
      a.play().then(() => {
        ...
        State.currentTrackInfo = info;
        updateNowPlayingBar(info);
        showToast('Appuie sur ▶ pour lancer');
      });
    };
  ...
  return {
    ...
    next() { if (State.queue.length) this._playIndex(State.queueIndex + 1); },
    prev() { if (State.queue.length) this._playIndex((State.queueIndex - 1 + State.queue.length) % State.queue.length); },
    pause()  { if (audio && !audio.paused) { audio.pause(); State.isPlaying = false; emit({ playing: false }); } },
    resume() { if (audio && audio.paused) { audio.play().then(() => { State.isPlaying = true; emit({ playing: true }); }).catch(()=>{}); } },
    toggle() { audio?.paused ? this.resume() : this.pause(); },

    stop() {
      queueMode = false;
      if (audio) { audio.pause(); audio.onended = null; audio.onerror = null; audio.src = ''; audio = null; }
      State.isPlaying = false; State.currentTrackInfo = null; State.queue = [];
      emit({ playing: false, track: null });
...
function updateNowPlayingBar(track) {
  const el = $('now-playing-title'); if (!el) return;
  el.textContent = track ? `${track.name} — ${track.artist}` : 'Lance une session pour écouter';
}
...
document.querySelectorAll('.tab[data-tab]').forEach(tab =>
  on(tab, 'click', () => { const MAP={play:'play',music:'music',sport:'sport'}; if(MAP[tab.dataset.tab]) navigateTo(MAP[tab.dataset.tab]); })
);
...
async function loadDeezerData() {
  // Chart Deezer pour écoutes récentes et recos
  const [chart1, chart2] = await Promise.all([
    DeezerManager.getChart(20),
    DeezerManager.getChart(16),
  ]);
...
function setBleUI(status, name='') {
  const dot=$('ble-dot'), text=$('ble-status-text'), chip=$('watch-chip'), lbl=$('ble-settings-label');
  if (status==='connected') {
...
BluetoothManager.onConnection(({ connected, deviceName }) => {
  ...
  if (!connected && State.bpmSource === 'watch') { State.bpmSource = 'simulator'; showToast('Montre déconnectée'); }
});
...
function buildQueueForBpm(bpm) {
  ...
  if (State.playlists.length) {
    let pl = null;
    if (bpm < 80)       pl = State.playlists.find(p => /bossa|jazz|chill|soul|slow|rnb/i.test(p.name));
    else if (bpm < 110) pl = State.playlists.find(p => /pop|rnb|r&b/i.test(p.name));
    else                pl = State.playlists.find(p => /rap|electro|hip|dance/i.test(p.name));
    if (!pl) pl = State.playlists[0];
    genreTracks = (pl?.tracks||[]).filter(t => t.previewUrl);
  }
  ...
  const maxLen = Math.max(recent.length, recos.length, 1);
  for (let i = 0; i < maxLen; i++) {
    if (i < recent.length) queue.push(recent[i]);
    if (i < recos.length)  queue.push(recos[i]);
    if (i < genres.length && i % 3 === 0) queue.push(genres[i]);
  }
  return queue.length ? queue : shuffle(getAllTracksWithPreview());
}

// Adapte la musique quand le BPM change de zone
let lastBpmZone = '';
...
function getAllTracksWithPreview() {
  return [...State.recentTracks, ...State.recoTracks, ...(State.playlists.flatMap(p=>p.tracks||[]))].filter(t=>t.previewUrl);
}
...
function updateBpmEverywhere(bpm, source, meta=null) {
  ...
  if (!meta) { const z=BpmSimulator.getZone(); meta={zone:z.label,color:z.color,bg:z.bg,conseil:conseilForBpm(bpm)}; }
  ...
  const cEl=$('conseil-text'); if(cEl&&meta.conseil) cEl.textContent=meta.conseil;
...
SensorsManager.onGps(({distance,positions}) => {
  ...
  const el=$('sport-distance'); if(el) el.textContent=`${km} km →`;
  updateMapTrace(positions);
});

function renderSteps(count) {
  const el=$('steps-count'), bar=$('steps-progress-bar');
  ...
  return `
    <div class="album-item" onclick="${onclick}" style="cursor:pointer">
      <div class="album-art" style="width:${size}px;height:${size}px;background:#222;position:relative">
        ${t.image?`<img src="${t.image}" alt="${esc(t.name)}" style="width:100%;height:100%;object-fit:cover;display:block">`:coverPlaceholder(t.name)}
        ${hasP?`<div style="position:absolute;bottom:4px;right:4px;width:20px;height:20px;background:rgba(0,0,0,.75);border-radius:50%;display:flex;align-items:center;justify-content:center"><svg viewBox="0 0 24 24" fill="white" width="10" height="10"><polygon points="5 3 19 12 5 21 5 3"/></svg></div>`:''}
      </div>
  ...
  return `
    <div class="history-item" onclick="${onclick}" style="cursor:pointer">
      <div style="width:100%;aspect-ratio:1;border-radius:6px;overflow:hidden;background:#222;position:relative">
        ${t.image?`<img src="${t.image}" alt="${esc(t.name)}" style="width:100%;height:100%;object-fit:cover">`:coverPlaceholder(t.name,'10px')}
        ${hasP?`<div style="position:absolute;bottom:3px;right:3px;width:16px;height:16px;background:rgba(0,0,0,.75);border-radius:50%;display:flex;align-items:center;justify-content:center"><svg viewBox="0 0 24 24" fill="white" width="8" height="8"><polygon points="5 3 19 12 5 21 5 3"/></svg></div>`:''}
      </div>
...
window._playGenre = function(idx) {
  ...
  if (!genre?.tracks?.length) { showToast('Chargement…'); return; }
...
on($('add-to-spotify'),  'click', async () => {
  ...
  showToast(pl ? 'Playlist créée sur Spotify ✓' : 'Erreur');
});
...
function renderPlaylists(genres) {
  ...
  if (genres?.length) {
    c.innerHTML = genres.slice(0,4).map((g,i) => `
      <div class="playlist-item" onclick="window._playGenre(${i})" style="cursor:pointer">
    ...
    c.innerHTML = SpotifyManager.getMockPlaylists().map(pl=>`
      <div class="playlist-item" style="cursor:pointer" onclick="showToast('Chargement…')">
...
function initSettingsScreen() {
  ...
  const slider=$('steps-goal-slider'), disp=$('steps-goal-display');
...
on($('spotify-connect-btn'), 'click', async () => {
  if (State.spotifyConnected) {
    SpotifyManager.logout(); Player.stop();
...
function updateSpotifySettingsUI(connected) {
  const lbl=$('spotify-status-label'), btn=$('spotify-connect-btn');
  if(!lbl||!btn) return;
